...
export const registerNestedLanguage = (
    language: Language,
...
export const loadLanguage = async (language: Language): Promise<void> => {
    if (languages[language] || generatedTranslations[language]) return
    ...
    const load = (
        language === 'zh'
            ? import('./langs/zh').then(({ default: catalog }) => {
                  registerNestedLanguage(language, catalog)
                  return null
              })
            : loadGeneratedTranslation(language)
    )
        .then((catalog) => {
