...
export const channelDocsHref = (
    ...
    /* The docs site carries its language in the path. Product surfaces want
       the default; the marketing pages, which are served per language, pass
       the one their own URL pins. */
    language: 'en' | 'zh' = 'en'
): string | null => {
    const path = channelDocsPaths[provider]
    if (!path) return null
    return docsHref(language === 'zh' ? `/zh${path}` : path)
}
