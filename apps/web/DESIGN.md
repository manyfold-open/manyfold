...
> **A migration is proposed against this document.** [`DESIGN.product-iris.md`](./DESIGN.product-iris.md) specifies moving the product register (webapp + docs) onto the landing system — Ash neutrals, the Iris brand ramp, and a serif display face — as [`DESIGN.landing.md`](./DESIGN.landing.md) §12 items 1–2 anticipated. **Until that lands, this document remains authoritative**: it describes the code as it is, the migration doc describes the target. Where they disagree, this one wins. Visual reference: `.product-iris.preview.html` at the repo root.
...
### 3.1 Where a value lives, and who decides

Three kinds of statement about the design system, three different owners.
Mixing them up is what let five separate copies of the same colour drift
apart, so this table is the first thing to read before changing anything.

| Statement                                                   | Single owner                                                       | Everything else                                                                       |
| ----------------------------------------------------------- | ------------------------------------------------------------------ | ------------------------------------------------------------------------------------- |
| **A value** — what number this token is                     | `packages/tokens`                                                  | No stylesheet, config or template may introduce a value the package does not declare  |
| **An intent** — why it is that number, when to reach for it | This document (product register) and `DESIGN.landing.md` (landing) | Code comments state the WHY and point here; they must not restate a figure — see §3.3 |
| **A constraint** — what must never happen                   | A gate                                                             | Documents may describe a constraint, but only a gate enforces one                     |

`pnpm tokens:check` verifies every consumer against the package — 346 values
across the four baselines. `pnpm tokens:drift` lists the divergences the
package holds on purpose.

**The baseline files, and what each may still decide for itself:**

| File                                               | Consumes from the package                                           | Still decides                                                                                                                                                       |
| -------------------------------------------------- | ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `packages/tokens/src/palette.ts`                   | —                                                                   | The Iris ramp and the Ash curve. **Both registers point at these steps**; this is what makes landing and the product one brand rather than two that happen to match |
| `packages/tokens/src/product-colors.ts`            | —                                                                   | Every `--color-*`, and the per-consumer overrides with their reasons                                                                                                |
| `packages/tokens/src/landing-colors.ts`            | —                                                                   | Every `--lp-*` except Fieldwork's 32, which are landing-only                                                                                                        |
| `packages/tokens/src/{scale,typography,shadow}.ts` | —                                                                   | Radius, font stacks, type ramps, shadow recipes                                                                                                                     |
| `apps/web/tailwind.config.ts`                      | **Imports** radius, font stacks, tracking, line-heights, weight cap | Which utilities to expose. It cannot disagree with the package — the gate asserts it keeps importing rather than comparing values that cannot differ                |
| `apps/web/src/styles.css`                          | Value-checked (cannot import JS)                                    | The selectors, the layer structure, and every rule that is not a token declaration                                                                                  |
| `apps/docs/src/styles/global.css`                  | Value-checked                                                       | Same, plus its own `@theme` wrapper — it is on Tailwind 4 and needs complete colour values where the webapp needs bare triplets                                     |
| `apps/api/.../email-content.ts`                    | **Imports** the palette                                             | Nothing about colour. Mail clients strip custom properties, so it needs literal hexes; the package derives them                                                     |
| `apps/admin/DESIGN.md` + admin styles              | **Not yet**                                                         | Admin is outside the package today. Bringing it in is open work                                                                                                     |

**Three colour syntaxes, one value.** This is why the values cannot simply
be shared as a file: `web` needs `244 244 246` (Tailwind 3 reads tokens
through `rgb(var(--x) / a)`), `docs` needs `rgb(244 244 246)` (Tailwind 4
`@theme` derives utilities from complete values), and `landing` is
hand-written CSS in `#f4f4f6`. The package emits each; maintaining three
spellings by hand is precisely how they came apart.
...
2. **Change the value in `packages/tokens`, never in a stylesheet.** The package is the only place a value is decided (§3.1). If the token does not exist yet, add it there first.
3. **Run `pnpm tokens:check`.** It compares all four baselines against the package and names the file, theme and token on any disagreement. This replaces the manual cross-check this step used to describe — the webapp and docs had already drifted on 16 colours by the time it was written, which is what a manual check is worth. If the two sides are _meant_ to differ, record it as an override with a reason instead of letting the values differ silently; `pnpm tokens:drift` lists what is currently held that way.

    Two things the package does not own: `--shadow-focus`, which the parent repo's `pnpm governance:check` parses into core and halo (it has to — the webapp spells it `rgb(var(--x) / a)` and docs must use `color-mix()`); and the admin app, which is not in the package yet.
...
- Never introduce ad-hoc hex values. Add a token to `packages/tokens` first, name its role here, then use it. `pnpm tokens:check` fails on a `--color-*` that appears in a stylesheet the package does not know about. The single documented exception is the landing-only LED-blue dot (§8.4).
