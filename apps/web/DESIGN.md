...
A tag is never interactive chrome. Two press-targets wear the tag anatomy, and both qualify on the same test — the tag _is_ the thing the press acts on, and it adds only hover opacity, no shadow, no new shape: the sandbox `SpriteStatusRefresh` control, which is the status it refreshes, and `VersionTag`, which is the version the update applies to. Anything that merely sits next to what it acts on is a button (§8.10), not a tag.

**A version and its update are one pill (`VersionTag`).** The technical-value tag that shows an installed version is also the product's one reminder that something newer exists: neutral while it is current, and when an update is out it takes the tone (`info`, or `error` when the update is required) and gains the rail's updates glyph (`CircleArrowUp`) as a trailing arrow that opens the Update Center. It never prints the available version beside the installed one — two version strings side by side make the reader diff them to learn the one thing a reminder is for. The version being offered belongs in the hover label (`ShortcutTooltip`, §8.7) and in the Update Center. A surface that shows its version as plain text rather than a pill (a dashboard cell, a caption) may stay text while it is current and become the pill when there is an update; what it must not do is grow a second chip.
