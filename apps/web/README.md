...
The static renderer inlines the complete Vite entry stylesheet on generated marketing pages, so their body can paint without a stylesheet round trip. Font URLs and rule order are unchanged. The original link uses `media="not all"`: it warms the immutable cache without blocking paint and lets Vite recognize the existing CSS dependency during later route loads. `app.html` keeps its normal external stylesheet. This trades a larger marketing HTML response for an earlier cold first paint; it does not trim selectors or postpone consent.
