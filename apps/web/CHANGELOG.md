...
- [#549](https://github.com/manyfold-open/manyfold/pull/549) [`43a601b`](https://github.com/manyfold-open/manyfold/commit/43a601bb3b7d0f63f4d450028dd4415c772a9202) Thanks [@yingca1](https://github.com/yingca1)! - Select or clear a whole group at once in the Update Center. Each group heading now has a checkbox that selects every update in that group that can be run from the page, and shows when only part of the group is selected; the table's select-all checkbox shows the partial state too. Selecting a collapsed group leaves it collapsed.

    A selected update that can no longer be run from the page, such as one on a machine that just went offline, now drops out of the selection instead of staying checked and counted. In right-to-left languages, table headings, group headings, the Update Center's action column and the Group by menu now line up on the reading side, and a collapsed group's arrow points the right way. The Japanese label of the Update selected button no longer contains a Simplified Chinese character.
...
- [#512](https://github.com/manyfold-open/manyfold/pull/512) [`7acadc9`](https://github.com/manyfold-open/manyfold/commit/7acadc9a6fbbbc9d8423aab1ff88233f2e4114f0) Thanks [@yingca1](https://github.com/yingca1)! - The four-step create flow now binds the provider picked at "Model cost" to a Claude Code, Codex or Gemini CLI agent. The agent joined its machine without one: on a sandbox it had no model credential at all, and on your own computer it silently ran on that computer's own sign-in instead of the key or managed billing you chose. The flow now binds the choice right after the agent joins, with the same model defaults the agent's settings would propose. A provider that was never tested, or that the CLI cannot talk to, stays listed with its reason and cannot be picked. On a machine that already runs agents, the step says that the ones billed to your account switch too, because that credential belongs to the machine.

## 2.3.1

### Patch Changes

- [#500](https://github.com/manyfold-open/manyfold/pull/500) [`a39ca39`](https://github.com/manyfold-open/manyfold/commit/a39ca39909f6e7e3b65d056a0b5b34a364674db9) Thanks [@yingca1](https://github.com/yingca1)! - The chat page no longer crashes on a model config view the browser cached before agents had runtime-auth bindings. Such an entry is discarded (the cache version is bumped and a cached view must carry its runtime-auth binding), and the page waits for the fresh view instead; before, the runtime sign-in card read the missing binding and the error boundary replaced the page on every reload.
...
- [#395](https://github.com/manyfold-open/manyfold/pull/395) [`eddfea1`](https://github.com/manyfold-open/manyfold/commit/eddfea1fcc889a50f462133522a7ac65a02a5e79) Thanks [@jiam1ngfu](https://github.com/jiam1ngfu)! - Agent creation gets a fourth variant, `v4`: four steps shown one screen at a
  ...
  and remain the default.
    ...
    OpenClaw and Hermes are installed at the last step, together with
...
- [#130](https://github.com/manyfold-open/manyfold/pull/130) [`3e82814`](https://github.com/manyfold-open/manyfold/commit/3e8281460f92b21631afa748353311d6554b6c42) Thanks [@jiam1ngfu](https://github.com/jiam1ngfu)! - The landing world's top plane now puts each framework on the runtime it
  actually runs on.
    ...
    The sandbox plate now carries a fourth framework, the cloud computer carries OpenClaw,
    ...
    `@lobehub/icons` has no mark for that framework, so the world borrows the product's own
...
- [#94](https://github.com/manyfold-open/manyfold/pull/94) [`69da61c`](https://github.com/manyfold-open/manyfold/commit/69da61c24ea6a02f0b002a7d38c79603752ebee2) Thanks [@yingca1](https://github.com/yingca1)! - The classic create-agent form takes the newer form's shape.
    ...
    **Model provider** is its own section instead of a dialog behind a "Configure
    ...
    says that too. Frameworks that manage providers in their own UI and the external ones do not get the control,
