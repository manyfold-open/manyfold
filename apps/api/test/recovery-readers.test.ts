...
test('parseClaudeJsonl skips the isMeta bookkeeping a slash command leaves as user entries', () => {
    const lines = [
        JSON.stringify({
            uuid: 'm1',
            parentUuid: null,
        ...
        JSON.stringify({
            ...
            type: 'user',
            isMeta: true,
            timestamp: '2026-09-17T20:00:00.100Z',
            message: {
                role: 'user',
                content:
        ...
        JSON.stringify({
            ...
            message: { role: 'user', content: 'Reply with exactly: pang' }
        }),
        JSON.stringify({
            uuid: 'a1',
            parentUuid: 'u1',
            sessionId: 'sess-1',
            type: 'assistant',
            timestamp: '2026-09-17T20:00:02.000Z',
            message: {
                role: 'assistant',
                content: [{ type: 'text', text: 'pang' }]
            }
        })
    ].join('\n')
    const { messages } = parseClaudeJsonl(lines, 'sess-1')
    assert.deepEqual(
        messages.map((m) => [m.role, m.externalId]),
        [
            ['user', 'u1'],
            ['assistant', 'a1']
        ]
