...
test('ensureRuntimeIdentity serializes concurrent first use and reuses the active token', async (t) => {
    ...
    try {
        ...
        assert.ok(created, 'one concurrent caller must create the identity row')
        ...
        assert.equal(active[0].id, created.runtimeTokenId)
        assert.equal(active[0].tokenHash, hashApiToken(created.plaintext))
    } finally {
        await h.close()
    }
})
