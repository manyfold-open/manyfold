...
const codingSurfaces: readonly ExecEnvSurface[] = [
    ...
    },
    {
        ...
        path: 'image-env',
        resume: 'none',
        note: 'The pod Secret also carries the key (the image entrypoint renders models.json from it), but the adapter re-sends it on every exec unconditionally, so the honest cell is per-exec.'
    },
    {
        framework: 'pi',
        runtime: 'k8s',
