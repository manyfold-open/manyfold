...
| Incoming files | Yes, when uploaded to Chat directly. Google Drive attachments are skipped — reading those needs a Drive scope the app does not hold. |
...
### Enable the API and create a service account
...
3. Go to **IAM & Admin → Service Accounts** and create a service account. It needs no project roles; the app's own identity is what matters.
4. Open the new account's **Keys** tab, add a key, and choose **JSON**. The file downloads once — treat it as a secret.

### Configure the app

1. In the Chat API page, open the **Configuration** tab.
2. Fill in the app name, avatar URL, and description.
3. Under **Functionality**, enable **Receive 1:1 messages** and **Join spaces and group conversations**.
4. Under **Connection settings**, choose **HTTP endpoint URL**. Leave the URL blank for now — Manyfold gives it to you in the next section.
5. Under **Visibility**, make the app available to yourself or to your domain.
...
4. Copy the channel's inbound URL and paste it into **Connection settings → HTTP endpoint URL** in the Chat API configuration page.

Add the app to a space, or open a direct message with it, and send a message.
