...
Windows 安装、指定目录、锁定版本见[安装 CLI](/zh/docs/install/)。
...
[profile](/zh/docs/profiles/)，而不是覆盖默认的那个。profile 在第一次被命名时就存在：
...
[注册 Self-owned computer](/zh/docs/local-daemons/) 完整讲了验证、自启、框架检测和吊销，
这些在自托管下同样适用，无需改动。
...
连接的反向代理会让注册成功、随后阻断 daemon 的 socket；见[自托管](/zh/docs/self-hosting/)
中关于对外提供服务的一节。
