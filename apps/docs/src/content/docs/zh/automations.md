...
description: 让 Agent 按照计划或工作流条件自动执行重复任务，把结果交付到 workspace、渠道或其他团队入口。
order: 8
---
...
- **团队提醒**：将完成情况或异常交付到 Slack、Telegram、Discord 等渠道。

## Automation、Agent 和渠道如何配合？

| 阶段 | 组件 | 负责什么 |
| ---- | ---- | -------- |
| **触发** | Automation | 决定 Agent 什么时候开始工作，例如定时计划或手动运行。 |
| **执行** | Agent + Workspace | 读取允许的文件和工具，执行 prompt，并生成可检查的结果。 |
| **交付** | 渠道或 workspace | 把报告、文件或状态发送到团队已经使用的入口。 |

简单理解：Automation 负责“什么时候做”，Agent 负责“做什么”，渠道负责“结果送到哪里”。

**想开始创建 Automation**？阅读[创建和管理 Automation](/zh/docs/automations/create/)；如果要把结果送到团队工具，再看[连接 Slack 渠道](/zh/docs/channels/slack/)。
