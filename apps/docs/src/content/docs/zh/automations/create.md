...
当前使用周期的运行额度耗尽后，定时运行会等待。Manyfold 会定期检查可用额度，并在恢复后安排下一个未来计划时间；提高额度或使用周期重置后，无需编辑 Automation。等待期间错过的运行不会补跑，手动暂停的任务也不会自动恢复。

对于旧版本中因配额而停止的计划，请先确认确实发生过配额跳过，再重新保存计划。仅凭较远的下次运行时间，无法判断它是否被配额阻止。
