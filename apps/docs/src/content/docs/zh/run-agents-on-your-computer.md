...
order: 6
---
**快速答案**：在 **Create agent → Advanced → Where it runs** 中选择 **Connect a new computer**，在目标电脑完成 `mf setup`，然后回到创建流程选择这台已连接的电脑。
...
2. 展开 **Advanced**，向下找到 **Where it runs**。
3. 点击 **Connect a new computer**。

![在创建 Agent 的 Where it runs 区块中，选择 Connect a new computer](../../../assets/docs/run-agents-on-your-computer/03-select-self-owned-computer-demo.webp)

*在创建流程中选择连接新的自有电脑。*

![Manyfold 的 Connect a new computer 视窗，显示 macOS 和 Linux 的安装命令及 Windows guide](../../../assets/docs/run-agents-on-your-computer/01-connect-new-computer.webp)
...
安装程序会开启浏览器，让你登录 Manyfold 并授权这台电脑。完成后，`mf` 会注册电脑、检测已安装的 coding agent，并启动本地 daemon。回到 Manyfold；当电脑显示绿色状态与 **connected** 时，点击 **Use this machine** 继续。
...
在 **Where it runs** 中选择刚连接、状态为 **Ready** 的 Self-owned computer。接着设置 Agent 的 **Workspace**，也就是它在该电脑上的工作目录。
...
| **Local config** | 使用这台电脑中所选 coding agent 已有的本地凭据，例如 CLI 登录会话、订阅或该 framework 使用的 API key。 |
...
![选择 Local config 作为模型来源，可重新检查本机可用的配置](../../../assets/docs/run-agents-on-your-computer/06-local-config-model-source.webp)

*Local config 会使用所选本地 framework 的可用凭据。*

如果 Local config 显示 **Not checked**，请重新检查状态，并确认对应的 Claude Code、Codex 或 Gemini CLI 已在该电脑正确安装和登录。若要使用团队或个人 API key，请在 Manyfold 的[模型提供商设置](/zh/docs/model-providers/)中添加、测试并保存 Provider。
...
- **我可以使用自己的 Codex 或 Claude Code 订阅吗？**
  ...
  可以。选择 Local config 后，Manyfold 会使用所选本地 framework 的凭据。请先确认 CLI 已安装、可运行且已登录；不同 framework 对订阅与 API key 的支持方式可能不同。
- **电脑必须一直开着吗？**

  是。当 Agent 需要在该电脑执行工作时，电脑必须开机、联网，且 `mf` daemon 必须运行。
- **可以连接多台电脑吗？**

  可以。每台电脑各自完成连接后，都会出现在 Where it runs 的清单中。创建 Agent 时选择它应运行的那一台即可。

## 另请参阅

- [学习 mf CLI](/zh/docs/cli/)
- [阅读 Local daemons](/zh/docs/local-daemons/)
- [Manyfold：团队 Agent 工作平台](https://manyfold.ai/)
