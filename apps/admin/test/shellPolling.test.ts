...
// refresh() is the post-mutation path: it must never be dropped by min
// spacing and must never issue a concurrent request.

test('refresh while idle runs immediately even within min spacing', async (t) => {
    const h = setup(t)
    h.controller.start()
    assert.equal(h.runs(), 1)
    await flush()
...
test('refresh during an in-flight run coalesces into exactly one follow-up', async (t) => {
    const h = setup(t)
    h.pendingMode(true)
    h.controller.start()
    assert.equal(h.runs(), 1)
    h.controller.refresh()
    h.controller.refresh()
    assert.equal(h.runs(), 1, 'never a concurrent request')
    h.pendingMode(false)
    h.resolvePending()
    await flush()
    assert.equal(h.runs(), 2, 'exactly one follow-up with fresh data')
    await flush()
    assert.equal(h.runs(), 2, 'the follow-up does not cascade')
})

test('refresh before start or after stop performs no run', async (t) => {
    const h = setup(t)
    h.controller.refresh()
    assert.equal(h.runs(), 0)
    h.controller.start()
